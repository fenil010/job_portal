...
export default function SearchRecommendations({
    ...
    return (
        <div className="bg-white rounded-2xl border-2 border-[#e8e0dc] overflow-hidden animate-fade-in-up">
            <div className="p-4 border-b-2 border-[#e8e0dc]">
                <div className="flex items-center gap-3">
                    <div className="p-2 bg-gradient-to-br from-[#789A99] to-[#5f7d7c] rounded-xl text-white">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                        </svg>
                    </div>
                    <h3 className="font-semibold text-[#1e2a32]">Jobs You Might Like</h3>
                </div>
            </div>
            ...
            {frequentTerms.length > 0 && (
                <div className="p-4 border-b-2 border-[#e8e0dc]">
                    <p className="text-xs text-[#8a9aa4] mb-2">Based on your searches</p>
                    <div className="flex flex-wrap gap-2">
                        {frequentTerms.map((term, idx) => (
                            <button
                                ...
                                className="px-3 py-1.5 text-xs font-medium bg-[#FFD2C2]/30 text-[#5a6b75] rounded-full hover:bg-[#789A99] hover:text-white transition-all"
                            >
                                {term}
                            </button>
                        ))}
            ...
            {recommendations.length > 0 && (
                <div className="divide-y divide-[#e8e0dc]">
                    {recommendations.map((job) => (
                        ...
                        >
                            <p className="font-medium text-[#1e2a32] text-sm mb-1">{job.title}</p>
                            <p className="text-xs text-[#8a9aa4]">{job.company} • {job.location}</p>
                            {job.skills && (
                                <div className="flex flex-wrap gap-1 mt-2">
                                    {job.skills.slice(0, 3).map((skill, idx) => (
                                        <span key={idx} className="px-2 py-0.5 text-xs bg-[#789A99]/10 text-[#789A99] rounded">
                                            {skill}
                                        </span>
                                    ))}
                                </div>
