...
const ToastItem = forwardRef(function ToastItem({ message, variant = 'default', title, duration = 5000, onClose }, ref) {
    ...
    return (
        ...
        bg-white rounded-xl shadow-xl shadow-[#1e2a32]/10 border-2 border-[#e8e0dc]
      ...
      `}
        >
            ...
            <div className="flex-1 min-w-0">
                {title && <p className="text-sm font-semibold text-[#1e2a32]">{title}</p>}
                <p className="text-sm text-[#5a6b75]">{message}</p>
            </div>
            <button
                type="button"
                onClick={handleClose}
                className="flex-shrink-0 p-1 text-[#8a9aa4] hover:text-[#1e2a32] hover:bg-[#FFD2C2]/30 rounded-lg transition-all duration-300"
                aria-label="Dismiss"
            >
